...
type Step = 'home' | 'selfie' | 'searching' | 'results' | 'sending' | 'sent'

export default function MiniWebPage() {
  ...
  const s: Record<string, React.CSSProperties> = {
    page: {
      minHeight: '100vh', background: '#0f0f1a', color: '#eee',
      fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, Roboto, sans-serif',
      padding: '20px', maxWidth: '480px', margin: '0 auto',
      display: 'flex', flexDirection: 'column',
    },
    card: { background: '#16213e', borderRadius: '14px', padding: '20px', marginBottom: '16px' },
    cardTitle: { fontSize: '1rem', fontWeight: 600, marginBottom: '12px', color: '#fff' },
    preview: {
      width: '180px', height: '180px', borderRadius: '50%',
      border: `3px ${selfiePreview ? 'solid' : 'dashed'} ${selfiePreview ? '#22c55e' : '#334155'}`,
    ...
    return (
      <div style={s.page}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
          ...
          <button
            style={{ ...s.btn, background: '#4f9cf7', color: '#fff', fontSize: '1.1rem' }}
  ...
  if (step === 'selfie') {
    return (
      <div style={s.page}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0', marginBottom: '16px' }}>
          <button style={{ background: 'none', border: 'none', color: '#4f9cf7', fontSize: '0.95rem', cursor: 'pointer' }} onClick={() => setStep('home')}>
            ← Back
          </button>
  ...
  if (step === 'results') {
    return (
      <div style={s.page}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '12px 0', marginBottom: '16px' }}>
          <h1 style={{ fontSize: '1.3rem', fontWeight: 700 }}>Results</h1>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: 'auto', paddingBottom: '20px' }}>
          <button style={{ ...s.btn, background: '#22c55e', color: '#fff' }} onClick={() => sendGroup('')}>
            📨 Send All to Telegram
          </button>
          <button style={{ ...s.btn, background: 'transparent', color: '#4f9cf7', border: '1px solid #4f9cf7' }}
  ...
  return (
    <div style={s.page}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '20px' }}>
        ...
        <button style={{ ...s.btn, background: '#4f9cf7', color: '#fff' }}
          onClick={() => { try { (window.Telegram as any)?.WebApp?.close() } catch {} }}>
          Close
        </button>
      </div>
    </div>
